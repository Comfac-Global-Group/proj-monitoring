...
| ID | datetime | Status | Issue | Notes |
|----|----------|--------|-------|-------|
| Q-035 | 260509-0000 | CLOSED | **Phone: Browser console panel overlaps Export/Download buttons in Settings** | **Symptom:** When DevTools/console is open on phone (needed for `store.addUser()` as per qa.md guide), the console panel compresses the viewport and the Settings panel's Export and Save As buttons scroll out of reach or land behind the DevTools UI. **RCA — most likely:** (1) `height: 100vh` on `.settings-panel` does not account for the DevTools/keyboard taking viewport height; the panel shrinks and overflow-y scroll may not clear the bottom obstruction on some Android browsers. (2) No `env(safe-area-inset-bottom)` or `padding-bottom` buffer in `.settings-panel` for system UI overlap. **Fix candidates:** Add `padding-bottom: env(safe-area-inset-bottom, 80px)` to `.settings-panel .settings-content`. Alternatively, document workaround: close DevTools before tapping Export. |
| Q-036 | 260509-0000 | CLOSED | **Phone: Service worker serving old cached version — v0.3.9 not visible** | **Symptom:** Phone shows an older version badge; v0.3.9 mobile fixes are not rendering. **RCA — most likely:** (1) `cache-first` strategy in `service-worker.js` serves old `app.js`/`styles.css`/`index.html` until the SW is fully replaced. (2) `showUpdateNotification()` only fires when `newWorker.state === 'installed' && navigator.serviceWorker.controller` — on a fresh phone install (no prior controller), the notification never fires, leaving stale content indefinitely. (3) PWA standalone mode removes the browser reload button; users cannot manually force-refresh. (4) The update notification renders at `top: 20px; right: 20px`; on narrow phones it may be partially off-screen or dismissed accidentally. **Fix candidates:** Add a "Check for update" button in Settings. Alternatively, bump APP_VERSION and redeploy to force cache invalidation. |
| Q-037 | 260509-0000 | CLOSED | **Phone: Settings panel and modals not opening (views don't show)** | **Symptom:** Settings panel and other interactive views do not appear when tapped on phone. **RCA — most likely (split-brain cache):** If the SW serves the old `app.js` while the browser delivers the new `index.html` (or vice versa), event listeners in `app.js` may reference DOM IDs that don't exist yet, causing silent failures — no settings panel, no modals. This is possible when the old SW is still active but some HTTP responses bypass it. **RCA — secondary (CSS regression in v0.3.9):** `styles.css` has two `@media (max-width: 768px)` blocks. The first (line 651) applies the v0.3.9 fix: `.project-header { flex-wrap: wrap }`. The second (line 1113) overrides it with `.project-header { flex-direction: column }`, reverting the fix. Combined with other layout issues, this could make the phone UI appear broken. **Fix candidates:** (1) Merge the two duplicate media query blocks and remove the conflicting rule. (2) Force a SW cache clear by incrementing `APP_VERSION` and redeploying. |
